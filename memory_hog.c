/*
 * memory_hog.c - Memory pressure workload for soft / hard limit testing (Task 4).
 *
 * Default behavior:
 *   - allocate 8 MiB every second
 *   - touch every page (memset) so RSS genuinely grows
 *
 * Usage:
 *   /memory_hog [chunk_mb] [sleep_ms]
 *
 * Build for Alpine rootfs (static so no glibc dependency inside chroot):
...
int main(int argc, char *argv[])
{
    ...
    while (1) {
        ...
        /* Touch every page so RSS actually grows - lazy allocation won't do */
