/*
 * io_pulse.c - I/O-oriented workload for scheduler experiments (Task 5).
 *
 * Usage:
 *   /io_pulse [iterations] [sleep_ms]
 *
 * Writes small bursts to a file and sleeps between them.
 * I/O-heavy processes voluntarily yield the CPU, letting the scheduler
 * demonstrate higher throughput for CPU-bound tasks when both are run
 * concurrently.
 *
 * Build for Alpine rootfs:
